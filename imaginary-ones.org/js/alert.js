...
setInterval(() => {
    if (isMobile()) return;
    for (let i = 0; i < getRdm(1, 3); i++) {
        let n = addNotification(NOTIFICATION_TYPES.MINT, `<i>0x${generateRandomString(3)}...${generateRandomString(3)}</i> just minted ${getRdm(1, 4)} NFTs!`);
        removeNotification(n, getRdm(1200, 2000))
    };
}, getRdm(3000, 6000));
